...
class HeaderPpidController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $headerPpid = Header::when($search, function ($query, $search) {
     ...
     */
    public function edit(Header $headerPpid)
    {
        $headerPpid = Header::findOrFail($headerPpid->id_header);
        $kategoriHeader = KategoriHeader::all();
        return view('Admin.ppid.headerPpid.formEditHeaderPpid', compact('headerPpid', 'kategoriHeader'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, Header $headerPpid)
    {
        $request->validate([
        ...
        if ($request->hasFile('gambar')) {
            ...
            $oldFilePath = 'header/' . $headerPpid->gambar;
            if ($headerPpid->gambar && Storage::disk('public')->exists($oldFilePath)) {

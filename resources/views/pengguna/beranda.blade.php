    ...
    @if($berita->isNotEmpty())
        ...
        <div class="d-none d-md-block">
            <a href="{{ route('berita.show', $unggulan->slug) }}" class="ds-featured-news text-decoration-none text-dark mb-4">
                <img src="{{ asset('storage/' . $unggulan->gambar1) }}" alt="{{ $unggulan->judul }}" class="ds-featured-img">
        ...
        <div class="d-md-none">
            <a class="ds-mfeat" href="{{ route('berita.show', $unggulan->slug) }}">
              <img src="{{ asset('storage/' . $unggulan->gambar1) }}" alt="{{ $unggulan->judul }}">
        ...
        @foreach($mitra->chunk(5) as $chunk)
            ...
            @foreach($chunk as $m)
                ...
                @if($m->link_mitra)
                    ...
                    <img src="{{ asset('storage/' . $m->gambar) }}" alt="{{ $m->nama_mitra }}">
                    </a>
                @else
                    <img src="{{ asset('storage/' . $m->gambar) }}" alt="{{ $m->nama_mitra }}">
                @endif
